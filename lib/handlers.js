	...
	if (tokenID) {
		_data.read('tokens', tokenID, (err, tokenData) => {
			if (!err && tokenData) {
				if (tokenData.expires > Date.now()) {
					_data.read('cart', tokenData.phone, (err, cartData) => {
						cartData.status = 'payment sucessful, wait for pizza';
						_data.update('cart', tokenData.phone, cartData, (err) => {
							if (!err) {
								_data.read('cart', tokenData.phone, (err, cartData) => {
									if (!err) {
										_data.read('users', tokenData.phone, (err, userData) => {
											...
											const price = parseInt(total.toFixed(2) * 100);
											const customer = `${userData.firstName} ${userData.lastName}`;
											helpers.purchase({ price, customer }, (recipeUrl) => {
												callback(200, {
													statusCode: 200,
												...
												helpers.SendMail(recipeUrl, cartData.cart, total, userData.email);
											});
										});
									} else {
										callback(500, {
